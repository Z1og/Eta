...
- 一次性完成 `jadx + apktool` 落盘并产出摘要：`scripts/decode.sh`
- Frida 设备检查、进程列举、spawn/attach 注入：`scripts/frida-run.sh`
- 重建、对齐、签名、安装 APK：`scripts/rebuild-sign-install.sh`
- 快速抽取 Manifest 关键组件与权限：`scripts/manifest-summary.sh`
...
```sh
sh -File "<skill-root>\apk-reverse\scripts\decode.sh" -ApkPath "D:\DOWNLOAD\app.apk" -Clean
sh -File "<skill-root>\apk-reverse\scripts\decode.sh" -ApkPath "D:\DOWNLOAD\app.apk" -Name demo -SkipJadx
```

### `scripts/frida-run.sh`
...
```sh
sh -File "<skill-root>\apk-reverse\scripts\frida-run.sh" -ListDevices
sh -File "<skill-root>\apk-reverse\scripts\frida-run.sh" -Usb -ListProcesses
sh -File "<skill-root>\apk-reverse\scripts\frida-run.sh" -Usb -Spawn -Package com.example.app -ScriptPath "D:\hooks\test.js"
```
...
```sh
sh -File "<skill-root>\apk-reverse\scripts\rebuild-sign-install.sh" -ProjectDir "C:\work\apktool_out" -Clean
sh -File "<skill-root>\apk-reverse\scripts\rebuild-sign-install.sh" -ProjectDir "C:\work\apktool_out" -Install -Reinstall -DeviceSerial "127.0.0.1:7555"
```
...
```sh
sh -File "<skill-root>\apk-reverse\scripts\manifest-summary.sh" -ManifestPath "C:\work\apktool_out\AndroidManifest.xml"
```
...
frida-trace -f com.example.app -j '*!*certificate*'
```
...
```sh
sh -File "<skill-root>\apk-reverse\scripts\rebuild-sign-install.sh" -ProjectDir "apktool_out" -Install -Reinstall -DeviceSerial "127.0.0.1:7555"
```
...
- 简单一次性命令直接用 `frida-*`
- 需要稳定复用的注入流程优先走 `scripts/frida-run.sh`
...
| 工具 | 可自动安装 | 安装方式 | 说明 |
|------|-----------|---------|------|
| jadx | ✓ | GitHub Release ZIP | 自动下载解压到 `/opt\jadx\` |
| apktool | ✓ | GitHub Release JAR + wrapper | 自动下载 jar 并生成 bat 到 `/opt\apktool\` |
| frida / frida-ps | ✓ | pip install frida-tools | 需要 Python 已安装 |
| adb | ✓ | apk add / fallback | 自动安装 Android Platform-Tools |
| zipalign | ✗ | 需手动安装 Android Build-Tools | `sdkmanager "build-tools;35.0.0"` |
| apksigner | ✗ | 需手动安装 Android Build-Tools | 同上 |

### 自举触发点

- `scripts/decode.sh`：缺 jadx 或 apktool 时自动调用 `bootstrap-reverse.sh`
- `scripts/rebuild-sign-install.sh`：缺 adb 或 apktool 时自动调用 bootstrap
- `scripts/frida-run.sh`：当前仍为手动检查（frida 通常已通过 pip 安装）

### 自举失败时

如果自动安装失败，脚本会抛出明确错误并附带手动安装链接。常见原因：
- 网络不通（GitHub API / PyPI 不可达）
- apk add 不可用（Alpine 无此包）
