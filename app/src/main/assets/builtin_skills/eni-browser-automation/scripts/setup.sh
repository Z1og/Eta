...
function Write-Step {
echo "[$((Get-Date).ToString('HH:mm:ss'))] $Message" 
}
...
if (-not $agentBrowser) {
    ...
    if ($LASTEXITCODE -ne 0) {
echo "[WARN] agent-browser global install failed. Trying npx fallback..."
    }
    else {
...
if (-not $SKIPBROWSERINSTALL) {
    ...
    if ($LASTEXITCODE -ne 0) {
echo "[WARN] Playwright browser install failed. You may need to run: npx playwright install chromium"
    }
    else {
    ...
    $playwrightPath = $null
        ...
        if ([string]::IsNullOrWhiteSpace($envBrowsersPath)) {
            ...
            if (test -e -LiteralPath $defaultPath) {
