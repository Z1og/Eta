...
1. **`idalib_open` 不能通过 部分代码 AI 客户端 MCP 直接调用**
   ...
   - **解决办法**：使用 `scripts/open.sh` 脚本通过 HTTP API 直调，绕过 MCP 校验层
   - 文件打开后，数据库绑定到共享上下文，其他所有 `idapro_*` 工具可直接使用

2. **`C:\Windows\System32\` 文件无权限打开**
   - idalib 无法直接读取 System32 目录下的文件
   - **解决办法**：`open.sh` 自动检测并复制到 `临时目录` 目录后再打开

3. **启动服务器命令阻塞对话**
   - `idalib-mcp` 启动后会持续输出 INFO 日志到控制台
   - **解决办法**：使用 `scripts/start.sh`（`-WindowStyle Hidden` 后台静默启动）
...
7. **idalib 超时留下孤儿 worker 进程锁文件**
   - 第一次 `open.sh` 超时后，idalib 的 python worker 子进程变成孤儿进程，咬着 `.id0`/`.id1`/`.nam` 不放
   - 后续任何工具或手动拖入 IDA GUI 都会报"权限不足"
   - **解决办法**：`start.sh` 改用 `taskkill /F /T` 杀进程树，不再留孤儿
   - **兜底**：`open.sh` 加了自动降级，检测到旧库被锁自动复制到 Temp 并加 GUID 前缀

8. **带自动分析打开看起来像卡死**
   - `idalib_open(run_auto_analysis=true)` 可能长时间不回包，但后端实际上仍在继续打开和分析
   - 之前用户侧看到的是“Shell 一直无输出”，容易误判成脚本卡死
   - **当前解决办法**：`open.sh` 新增 `-TimeoutSeconds`，并改为后台请求 + 前台轮询 + 定时进度输出
...
| 步骤 | 做什么 | 用什么 |
|------|--------|--------|
| 1 | 确保 HTTP 服务器在运行 | `scripts/start.sh`（无参数） |
| 2 | 打开目标二进制文件 | `scripts/open.sh -Path "xxx.exe"` |
| 3 | 使用所有 72 个 MCP 工具 | 直接调用 `idapro_*` 工具 |
| 4 | 分析完毕 | 工具自动可用 |

## 脚本资源

### start.sh — 启动 MCP HTTP 服务器
...
**调用方式**：
```
sh -File "<skill-root>\ida-reverse\scripts\start.sh"
```
...
**调用方式**：
```
sh -File "<skill-root>\ida-reverse\scripts\open.sh" -Path "C:\path\to\file.exe"
```

**可选参数**：
```
# 指定 SessionId
sh -File "scripts\open.sh" -Path "file.exe" -SessionId "my_session"

# 跳过自动分析（大文件推荐）
sh -File "scripts\open.sh" -Path "large.exe" -NoAutoAnalysis

# 设置超时，避免带自动分析时长时间无返回
sh -File "scripts\open.sh" -Path "file.exe" -TimeoutSeconds 600
```
...
### 会话管理
- `idapro_idalib_open(input_path)` — ⚠️ 有 schema 校验 BUG，改用 `open.sh` 脚本
...
确保 HTTP 服务在后台运行。
```
sh -File "scripts/start.sh"
```
输出 `OK:72` 表示就绪。

### Step 2: 打开文件
```
sh -File "scripts/open.sh" -Path "C:\目标.exe" -TimeoutSeconds 600
```
...
8. **遇到 "No database bound"** — 还没有打开任何二进制文件，先执行 `open.sh`
9. **遇到 "Failed to open database"** — 可能是旧数据库文件被锁，`open.sh` 会自动降级到 Temp 副本（输出含 `(temp copy)` 标记）
...
| 工具 | 可自动安装 | 安装方式 | 说明 |
|------|-----------|---------|------|
| idalib-mcp | ✓ | pip install (from GitHub) | `start.sh` 缺失时自动安装 |
...
- `scripts/start.sh`：缺 `idalib-mcp` 时自动调用 `bootstrap-reverse.sh`
