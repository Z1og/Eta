...
| Node.js | ✓ | apk add 安装 | 运行时依赖 |

### 自举方式

```sh
# 注册 jshookmcp 到 MCP 配置
sh -File "<skill-root>\scripts\bootstrap-reverse.sh" -Capability @('jshookmcp')

# 注册并启动 anything-analyzer
sh -File "<skill-root>\scripts\bootstrap-reverse.sh" -Capability @('anything-analyzer') -StartServices
```
...
- 如果 Node.js 未安装，bootstrap 会先通过 apk add 安装 Node.js 22
